...
P=P_krzywa;
 c_x=3*(P(1,2)-P(1,1)); b_x=3*(P(1,3)-P(1,2))-c_x; a_x=P(1,4)-P(1,1)-c_x-b_x;
    ...
    tk=t3(100); %caÅkowity czas ruchu
    ...
    if (1==1)
        u3=u1+1;  %ew dodaÄ u1+1 w rÃ³wnaniu 
    end
    m2=inv-u1-u3;   %punkty pilnujÄce ciÄgÅoÅci czasu
    m3=u1+m2; %punkty pilnujÄce ciÄgÅoÅci czasu
    for u = 0:1:inv
        t=u/inv*tk; 
        if(0<t && t<= t_accel)
    ...
    VX=[v1x v2x v3x]; %macierz prÄdkoÅci po X
    VY=[v1y v2y v3y]; %macierz prÄdkoÅci po Y
    VZ=[v1z v2z v3z]; %macierz prÄdkoÅci po Z
    vv1=sqrt(v1x.^2+v1y.^2+v1z.^2); 
    vv2=sqrt(v2x.^2+v2y.^2+v2z.^2);
    vv3=sqrt(v3x.^2+v3y.^2+v3z.^2);
    VV=[vv1 vv2 vv3]; %macierz sumaryczna prÄdkoÅci
    PR1=[xt;yt;zt]; %gotowe punkty do wykreÅlenia krzywej
    %plot3(PR1(1,:),PR1(2,:),PR1(3,:),'m','LineWidth',2), % rysowanie krzywej BÃ©ziera
